...
class ProstheticsMetricProGait(CocoMetric):
    ...
    def compute_metrics(self, results):
        ...
        for res in results:
            ...
            valid_mask = (g_types >= 0) & (g_types <= 2)
        ...
        for res in results:
            ...
            for k in range(num_kpts):
                ...
                if k in LIMB_AND_RES_KPTS:
                    ...
                    # Absent/Missing keypoints should still be counted for type accuracy.
                    type_valid = 0 <= gt_t <= 2 and 0 <= pr_t <= 2
                    ...
                    if type_valid:
                        ...
                        all_y_true.append(int(gt_t))
                        all_y_probs.append(prob_arr)
                        kpt_y_true[k].append(int(gt_t))
                        ...
                        if gt_t == 0:
                            m['norm_counts'][k] += 1
                            m['norm_correct'][k] += (pr_t == 0)
                        elif gt_t == 1:
                            m['pros_counts'][k] += 1
                            m['pros_correct'][k] += (pr_t == 1)
                        elif gt_t == 2:
                            m['miss_counts'][k] += 1
                            m['miss_correct'][k] += (pr_t == 2)

                        # Include residual limbs 23-30 in confusion matrix
                        m['confusion'][int(gt_t), int(pr_t)] += 1
                # 回归统计
                if gt_t == 2: v_g = 0
                if v_g > 0:
