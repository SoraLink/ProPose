...
class ProstheticsBottomUpMetric(CocoMetric):
    ...
    def _match_instances(self, results):
        ...
        for res in results:
            inst = res[0]
            pred_bboxes = torch.as_tensor(inst.get('pred_bboxes', []))
            gt_bboxes = torch.as_tensor(inst.get('gt_bboxes', []))
            ...
            iou_mat = box_iou(pred_bboxes, gt_bboxes).numpy()

            # 匈牙利匹配 (求最大 IoU 总和)
            p_indices, g_indices = linear_sum_assignment(-iou_mat)

            for p_idx, g_idx in zip(p_indices, g_indices):
                if iou_mat[p_idx, g_idx] < self.iou_thr:
                    continue  # 剔除误检 (幻觉框)

                # 组装成功配对的单一实例数据 (格式与以前的 [0] 类似)
                area = inst['gt_instances']['areas'][g_idx] if 'areas' in inst['gt_instances'] else \
                    (gt_bboxes[g_idx][2] - gt_bboxes[g_idx][0]) * (gt_bboxes[g_idx][3] - gt_bboxes[g_idx][1])

                matched_inst = {
                    'pred_kpts': inst['keypoints'][p_idx],
                    'pred_type': inst['pred_types'][p_idx].copy(),
                    'pred_score': inst['type_scores'][p_idx],
                    'gt_kpt': inst['gt_instances']['keypoints'][g_idx],
                    'gt_vis': inst['gt_instances']['keypoints_visible'][g_idx],
