...
class ProstheticsDEKRMetric(CocoMetric):

    def __init__(self,
                 score_thr=0.3,  # 判定幻觉的置信度阈值
                 oks_thr=0.3,  # 🌟 将 iou_thr 替换为 oks_thr
                 **kwargs):
        super().__init__(**kwargs)
        self.score_thr = score_thr
        self.oks_thr = oks_thr  # 🌟 保存 OKS 阈值
    ...
    def _match_instances(self, results):
        ...
        for res in results:
            ...
            gt_areas = inst.get('gt_areas', [])

            num_preds = len(pred_kpts)
            num_gts = len(gt_kpts)

            # 如果没人预测出来，或者没有真实标签，跳过
            if num_preds == 0 or num_gts == 0:
                continue

            # 初始化 OKS 矩阵 [Num_Preds, Num_GTs]
            oks_mat = np.zeros((num_preds, num_gts))

            # 计算 OKS (完全抛弃 BBox)
            for p_idx in range(num_preds):
                for g_idx in range(num_gts):
                    vis_mask = gt_vis[g_idx] > 0
                    if np.sum(vis_mask) == 0 or gt_areas[g_idx] <= 0:
                        oks_mat[p_idx, g_idx] = 0.0
                        continue

                    # 计算预测点与真实点的欧式距离平方
                    d_sq = np.sum((pred_kpts[p_idx][vis_mask] - gt_kpts[g_idx][vis_mask]) ** 2, axis=1)
                    k_sq = sigmas[vis_mask] ** 2

                    area = gt_areas[g_idx]
                    if isinstance(area, torch.Tensor):
                        area = area.item()

                    # 标准 OKS 公式计算：exp(-d^2 / (2 * area * k^2))
                    e = np.exp(-d_sq / (2 * area * k_sq + 1e-9))
                    oks = np.sum(e) / np.sum(vis_mask)
                    oks_mat[p_idx, g_idx] = oks

            # 匈牙利匹配 (求最大 OKS 总和)
            p_indices, g_indices = linear_sum_assignment(-oks_mat)

            for p_idx, g_idx in zip(p_indices, g_indices):
                if oks_mat[p_idx, g_idx] < self.oks_thr:
                    continue  # 剔除匹配度过低的幻觉人体或误匹配

                area = gt_areas[g_idx]
                matched_inst = {
                    'pred_kpts': pred_kpts[p_idx],
                    'pred_type': inst['pred_types'][p_idx].copy(),
                    'pred_score': inst['type_scores'][p_idx],
                    'gt_kpt': gt_kpts[g_idx],
                    'gt_vis': gt_vis[g_idx],
                    'gt_type': inst['gt_types'][g_idx],
                    'area': area.item() if isinstance(area, torch.Tensor) else area
                }
                matched_list.append(matched_inst)
    ...
    def compute_metrics(self, results):
        ...
        matched_results = self._match_instances(results)
        ...
        for match in matched_results:
            p_types = match['pred_type']
            g_types = match['gt_type']
            vis = match['gt_vis']
        ...
        self.report_custom_stats(matched_results)
    ...
    # 模块 1：分层启发式后处理 (核心逻辑)
    # =========================================================================
    def _apply_post_processing_to_results(self, matched_results):
        omega_dict = {
            23: [7, 9, 17, 25],
            24: [8, 10, 18, 26],
            25: [9, 17, 23],
        ...
        for match in matched_results:
            gt_v = match['gt_vis']
            pred_types_before = match['pred_type']
            pred_type_scores = match['pred_score']

            # 保存原始预测
            match['pred_types_before'] = pred_types_before.copy()
            ...
            match['pred_types_after'] = pred_types_after
    ...
    # =========================================================================
    def _calculate_metrics(self, matched_results, type_key):
        import numpy as np
        ...
        for match in matched_results:
            pred_kpts = match['pred_kpts']
            gt_kpt = match['gt_kpt']
            gt_v = match['gt_vis']
            gt_types = match['gt_type']
            pred_types = match[type_key]
            type_scores = match['pred_score']
