...
class FocalAnatomyAwareHead(HeatmapHead):
    def __init__(self,
    ...
    def loss(self, feats, batch_data_samples, train_cfg=None, **kwargs):
        ...
        ce_mask = (visible_flat > 0).float()
        ce_mask = ce_mask.view(-1)
        raw_ce_loss = F.cross_entropy(logits_flat, types_flat.view(-1), reduction='none')
        pt = torch.exp(-raw_ce_loss)
        gamma = 2.0
        focal_weight = (1.0 - pt) ** gamma
        loss_type = (raw_ce_loss * focal_weight * ce_mask).sum() / (ce_mask.sum() + 1e-6)
